...
function pprint.pformat(obj, option, printer)
    ...
    local function table_formatter(t)
        ...
        local function print_kv(k, v, t)
            ...
            if
                formatter[type(v)] == nop_formatter or formatter[type(k)] == nop_formatter or
                    (option.filter_function and option.filter_function(v, k, t))
             then
                return
            end
            wrapped = _n()
    ...
    formatter["function"] =
        option.show_function and make_fixed_formatter("function", option.object_cache) or nop_formatter
    formatter["thread"] = option.show_thread and make_fixed_formatter("thread", option.object_cache) or nop_formatter
    formatter["userdata"] =
        option.show_userdata and make_fixed_formatter("userdata", option.object_cache) or nop_formatter
